...
"TPLinkHS110: $name Get called. Relay state: $json->{'system'}->{'get_sysinfo'}->{'relay_state'}, RSSI: $json->{'system'}->{'get_sysinfo'}->{'rssi'}";
    ...
    foreach my $key ( sort keys %{ $json->{'system'}->{'get_sysinfo'} } ) {
        readingsBulkUpdate( $hash, $key,
            $json->{'system'}->{'get_sysinfo'}->{$key} );
    }
...
"TPLinkHS110: $name: Received zero bytes of realtime data. Cannot process realtime data";
        ...
        eval { $realtimejson = decode_json($rdata); } or do {
            Log3 $hash, 2,
"TPLinkHS110: $name json-decoding failed. Problem decoding getting statistical data";
            return;
        };
        ...
        {
            readingsBulkUpdate( $hash, $key2,
                $realtimejson->{'emeter'}->{'get_realtime'}->{$key2} );
        }
        Log3 $hash, 3,
"TPLinkHS110: $name Device is an HS110. Got extra realtime data: $realtimejson->{'emeter'}->{'get_realtime'}->{'power'} Watt, $realtimejson->{'emeter'}->{'get_realtime'}->{'voltage'} Volt, $realtimejson->{'emeter'}->{'get_realtime'}->{'current'} Ampere";
...
sub TPLinkHS110_Set($$) {
    ...
    return "Unknown argument $a[1], choose one of <on> <off> <toggle> "
      if ( $a[1] ne "on" & $a[1] ne "off" & $a[1] ne "toggle");
    ...
    my $json;
    eval { $json = decode_json($data); } or do {
        Log3 $hash, 2, "TPLinkHS110: $name json-decoding failed. Problem decoding statistical data";
        return;
    };
